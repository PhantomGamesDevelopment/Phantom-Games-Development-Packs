...
function createAndConnectToLocalServer( %serverType, %level ) {
	MessagePopup("Please Wait", "Creating Server and Loading Game Datablocks, Please Wait...");
	if( !createServer( %serverType, %level ) ) {
      closeMessagePopup();
		MessageBoxOK("Error", "Failed to create a local server.");
		return false;
	}
...
function createServer(%serverType, %level) {
   ...
   if (%serverType $= "MultiPlayer") {
      ...
      $Server::InPreGame = true;
      $Server::LevelSelection = %level;
      $Server::isMultiplayerServer = 1;
	  //Phantom139: Don't load just yet!
      ...
      $Server::isMultiplayerServer = 0;

      //Phantom139: We load if it's single player
      loadMission(%level, true);		
   }  
   return true;
}
