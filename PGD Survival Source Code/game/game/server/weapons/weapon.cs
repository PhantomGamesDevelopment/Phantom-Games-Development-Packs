...
// Load The Weapons
exec("./commonBlocks.cs"); //common Datablocks
exec("./grenade.cs");      //grenade & General Explosions
...
function WeaponImage::onMount(%this, %obj, %slot) {
   ...
   if(%this.isField("clip")) {
      ...
      if (%obj.getInventory(%this.ammo)) {
         ...
         %obj.setInventory(%this.ammo, %obj.maxInventory(%this.ammo));
         %obj.setImageAmmo(%slot, true);
         
         // Add any spare ammo that may be "in the player's pocket"
         %currentAmmo = %obj.maxInventory(%this.ammo);
      ...
      %amountInClips *= %obj.maxInventory(%this.ammo);
...
function WeaponImage::reloadAmmoClip(%this, %obj, %slot)
{
	...
	%spec = %obj.spec(%weaponID);
	if(%spec $= "Bandolier III") {
	   %infClips = true;
	}
...
function WeaponImage::stashSpareAmmo( %this, %player )
{
   ...
   // on the player
   if (%player.getInventory( %this.ammo ) < %player.maxInventory(%this.ammo) )
   {
      ...
      if ( %combinedAmmo >= %player.maxInventory(%this.ammo) )
      {
         %player.setFieldValue( %nameOfAmmoField, %combinedAmmo - %player.maxInventory(%this.ammo) );
      ...
      if ((%image = %obj.getMountedImage(%i)) > 0)
         if (isObject(%image.ammo) && %image.ammo.getId() == %this.getId())
         {
            ...
            if (%obj.getClassname() $= "Player")
            {
               ...
               {
                  %amountInClips = %obj.getInventory(%this.clip);
                  %amountInClips *= %obj.maxInventory(%this);
