...
function GameCore::createGame()
{
   ...
   if ($Server::MissionType $= "")
      $Server::MissionType = "Survival"; //Default gametype, just in case
...
package GameCore
{
   ...
   function GameConnection::onConnect(%client, %name) {
      ...
      //have the client send specializations
		commandToClient(%client, 'requestSpecializations');
      ...
      echo("CADD: "@ %client @" "@ %client.getAddress());
		...
		//Phantom139: Check for Pre-Game
		if($Server::InPreGame) {
		   PreGameDetails.addToPreGameLobby(%client);
		}
		else {
   ...
   function GameConnection::onDrop(%client, %reason) {
      ...
      $Server::PlayerCount--;
		...
		if($Server::InPreGame) {
		   PreGameDetails.removeFromPreGameLobby(%client);
		}
...
function GameCore::onClientEnterGame(%game, %client)
{
   ...
   messageClient(%client,
      'MsgClientJoin', '\c2Welcome to Survival %1.',
      %client.playerName,
      %client,
      %client.sendGuid,
      %client.team,
...
function GameCore::loadOut(%game, %player) {
   ...
   %client.setWeapons(Ryder, "", 1, 0); //Default Starting Value
}
