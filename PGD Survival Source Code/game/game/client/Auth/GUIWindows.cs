...
function MainMenuGui::onWake(%this) {
   ...
   VersionText.setText("<Font:Arial:16><Color:FFFFFF>"@$GameVersion);
   COMMOD_INDIC.setVisible(false);
   if($RETPRELIMINARY >= 1) {
      COMMOD_INDIC.setVisible(true);
      if($RETPRELIMINARY == 2) {
         COMMOD_INDIC.tooltip = "Developer Account";
      }
      else {
         COMMOD_INDIC.tooltip = "Community Moderator Account";
      }
   }
   //Load the client's info
   clientStartup();
   //First Time?
   if(!weaponUnlocked(0)) {
      MessageBoxOK("Welcome", "Welcome to PGD Survival!\nBefore jumping into the game be sure to set your controls and visit 'The Armory'.");
      //
      %guid = $ConnStore::guid;
...
function LoginWindow::doLogin(%this) {
   ...
   if(%accID == 0) {
      ...
      if(%attempt !$= "INVALID_PASSWORD") {
         ...
         $Pref::Server::Name = $ConnStore::username;
...
function AccountRegistrationWindow::onWake(%this) {
   ...
   //check the server
   %_OK = "game/data/image/guiIcons/tickMark_OK.png";
...
function AccountRegistrationWindow::OnUpdateFields(%this) {
   %_BAD = "game/data/image/guiIcons/tickMark_BAD.png";
   %_WARN = "game/data/image/guiIcons/tickMark_WARN.png";
   %_OK = "game/data/image/guiIcons/tickMark_OK.png";
