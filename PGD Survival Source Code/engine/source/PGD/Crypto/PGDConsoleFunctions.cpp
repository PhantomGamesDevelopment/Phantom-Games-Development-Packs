...
//Phantom139: Added debug function to test for active packages
	/**
DefineEngineFunction(testPGDPackages, void, (),, "") {
	Con::printf("Testing active PGD Packages...");
	std::string statusM;
	statusM = communityModerator == NULL ? "Disabled" : "Enabled";
	Con::printf(" * PCM: %s", statusM.c_str());
	statusM = cryptoPackage == NULL ? "Disabled" : "Enabled";
	Con::printf(" * XXZ568: %s", statusM.c_str());
	statusM = CCNData == NULL ? "Disabled" : "Enabled";
	Con::printf(" * CCNData: %s", statusM.c_str());
	statusM = pgs_main == NULL ? "Disabled" : "Enabled";
	Con::printf(" * RankStore: %s", statusM.c_str());
}
**/

DefineEngineFunction(checkCurse, bool, (const char * input),,"") {
   return true;
}
...
/**
DefineEngineFunction(checkCurse, bool, (const char * input),, "checks input for invalidations") {
   const char *curseList[] = {"fuck", "shit", "ass", "nigger", "dick", "vagina", "balls", "whore", "bitch", "penis"};

   bool invalid = true;
   for(int i = 0; i < sizeof(curseList); i++) {
      if(strtok((char *)input, curseList[i]) != NULL) {
         invalid = false;
      }
   }
...
DefineEngineFunction(DecryptAccount, const char *, (const char *data, const char *decHash, const char *key),, "(string) decrypts an account running AES-CBC cipher") {
   ...
   //Con::printf("%s (%s)\n%s", cryptoPackage->pgdHash(holderStringDec).c_str(), holderStringDec.c_str(), decodeHex.c_str());
   //fileobject test
...
DefineEngineFunction(GatherAccountDetails, void, (const char *det, const char *E, const char *N, const char *S),, 
   ...
   if(rsaverifyresult != true) {
	   ...
	   Con::evaluatef("schedule(250, 0, disconnect);");
	   return;
   }
