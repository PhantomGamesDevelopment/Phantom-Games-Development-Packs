...
void PGDStore::create() {
   if(pgs_main == NULL) {
	   pgs_main = new PGDStore();
		Con::printf("PGD Module Loaded: PGS");
	}
}
...
void PGDStore::destroy() {
	if(pgs_main != NULL) {
	   delete pgs_main;
		pgs_main = NULL;
		Con::printf("PGD Module Removed: PGS");
	}
}
...
void PGDStore::_PGS_Value::set_value(std::string v) {
	cryptoPackage->AESEncrypt(cryptoPackage->pgdHash("Phantom Games Development"), v, value, 512);
}

std::string PGDStore::_PGS_Value::get_value() {
	std::string out;
	cryptoPackage->AESDecrypt(cryptoPackage->pgdHash("Phantom Games Development"), value, out, 512);
	return out;
}
...
void PGDStore::saveToFile(const char *filePath) {
   _PGS_Value *tmp = chain_head;
	...
	cryptoPackage->AESEnc_File(cryptoPackage->pgdHash("Phantom Games Development"), filePath, 1024);
}
...
void PGDStore::loadFromFile(const char *filePath) {
   _PGS_Value *tmp = chain_head;
	bool fullLoad = false;
	//decrypt the file
	cryptoPackage->AESDec_File(cryptoPackage->pgdHash("Phantom Games Development"), filePath, 1024);
	...
	while(!file_in.eof()) {
	   getline(file_in, line);
		...
		//decrypt the value
		cryptoPackage->AESDecrypt(cryptoPackage->pgdHash("Phantom Games Development"), newValue, f_newValue, 512);
		//add the PGS Entry
