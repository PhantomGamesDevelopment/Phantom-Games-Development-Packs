   ...
   switch($mode) {
      ...
      case 2:
         ...
         if($ok == 1) {
            ...
            $stmt1 = mysqli_prepare($con, 'INSERT INTO $AT (guid, E, N, D, D_DEC_HASH, signature, username, email, hashpass) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
            mysqli_stmt_bind_param($stmt1, 'sssssssss', $guid, $exp, $public, $private, $priv_dec_hash, $sigFinal, $name, $email, $accountData);
            if(!sqlCatchResult($stmt1)) {
               die("$MAP"."INTERNAL_ERROR");
            }

            $stmt2 = mysqli_prepare($con, 'DELETE FROM $AK WHERE AccKey=? AND Email=?');
            mysqli_stmt_bind_param($stmt2, 'ss', $key, $email);
            if(!sqlCatchResult($stmt2)) {
               die("$MAP".'INTERNAL_ERROR');
            }
            //sqlDC($result2);
            sqlDC($con);         
