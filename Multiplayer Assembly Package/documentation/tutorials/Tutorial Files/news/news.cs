...
function newsDlg::onWake(%this) {
   if(!isObject(newsControl)) {
      MessagePopup("News", "Please Wait, Connecting to News Server");
...
function downloadNews() {
   ...
   %header = assembleHTTP1_1Header("www.mywebsite.com", "/path/to.news.php", "POST", "MAP Client", "Content-Type: multipart/form-data; boundary="@%separator@"\r\n");
   %dispo = makeDisposition(%separator, newsReq, 1, 1);
   %header = %header @ "Content-Length: " @ strLen(%dispo) @ "\r\n\r\n";
   
   %theNews.request = %header @ %dispo;
   %theNews.connect("www.mywebsite.com:80");
}
...
function NewsDownloader::onLine(%this, %line) {
   closeMessagePopup();
   if(strstr(%line, "$MAP") != -1) {
      %line = strReplace(%line, "$MAP", "");
