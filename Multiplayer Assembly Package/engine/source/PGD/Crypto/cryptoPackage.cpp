...
//http://www.phantomdev.net/certAuthorityCreate.php (Public Key Hex goes here)
const char * xxz568::caPublic() {
   return "Paste_CA_Public_Key_Here";
}

//obviously change me :D
const char * xxz568::AESFileKey() { return "MyUltraSecretAESKey"; }
...
int xxz568::AESEnc_File(std::string key, std::string path, unsigned int PBKDFiterations, std::string outFile) {
   ...
   std::ofstream fileEdit(outFile.c_str(), std::ios::out);
	fileEdit << "M.A.P. Encrypted File (AES-256-CBC): " << path.c_str() << "\n";
...
bool xxz568::isEncryptedFile(const char *path) {
   //open the file and run the first line through to see if it contains the tag
	...
	if(line.substr(0, 36).compare("M.A.P. Encrypted File (AES-256-CBC):") == 0) {
	   return true;
	}
	return false;
}
...
void xxz568::generateRSAKey() {
   ...
   while(accountRSA.Validate(rng, 3) == false) {
      accountRSA.GenerateRandomWithKeySize(rng, RSA_KEY_SIZE);
...
String xxz568::recoverAccount(const char *username, const char *password) {
	...
	if(strcmp(Con::getVariable("$DebugMode"), "1") == 0) {
		Con::printf("DEBUG:\nServer: %s\nRequest: %s", server.c_str(), data.c_str());
	}

	String recover = tcpBP->readURL(server.c_str(), data.c_str());
	String FirstWord = StringUnit::getUnit( recover, 0, " \t\n");
	if(FirstWord.compare("$MAP$INCORRECT_PASS") == 0 || FirstWord.compare("$MAP$NO_SUCH_ACCOUNT") == 0) {
	   Con::executef("MessageBoxOK", "Error", "Invalid Username or Password");
	}
	else if(FirstWord.compare("$MAP$RECVR") == 0) {
		recover = recover.replace("$MAP$RECVR ", "");
...
String xxz568::registerAccount(const char *username, const char *password, const char *email, const char *key) {
   ...
   try {
	...
	if(FirstWord.compare("$MAP$BAD_NAME") == 0) {
	...
	else if(FirstWord.compare("$MAP$SIGN_FAILURE") == 0) {
	   ...
	   Con::executef("MessageBoxOK", "Error", "The server has encountered an internal error.\nPlease try again later.");
		...
		//Phantom139: Proceed with login, push a thanks message and go from here!
		String publicCert = un;
		publicCert += "\t";
		publicCert += guid;
		publicCert += "\t";
		publicCert += email;
		...
		publicCert += rsasig;
		//
		Con::executef("Canvas.popDialog", "AccountRegistrationWindow");
		Con::executef("StoreSuccessfulLogin", e.c_str(), n.c_str(), rsasig);
		Con::setVariable("$ConnStore::username", un.c_str());
		Con::setVariable("$ConnStore::email", email);
		Con::setVariable("$ConnStore::guid", guid.c_str());
		Con::setVariable("$ConnStore::sig", rsasig.c_str());
		Con::setVariable("$PublicCert", publicCert.c_str());
		Con::executef("Canvas.setContent", "MainMenuGui");
		//
		Con::executef("MessageBoxOK", "Registration Successful", "Account Registration Sucessful, Thank you for registering!");
	}
	else {
		Con::executef("MessageBoxOK", "Error", "Account Server Returned an Unrecognized Command\nPlease send console.log to development team if problem persists.");
